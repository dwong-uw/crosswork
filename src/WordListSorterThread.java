...
public class WordListSorterThread extends Thread {
    ...
    public WordListSorterThread(DefaultListModel<XWord> list, CrossworkBoardState model, WordFinder finder,
            boolean focused_isHorizontal, int focused_x, int focused_y, CrossworkProgressListener cpl) {
        this.list = list;
        this.model = model;
        this.finder = finder;
    ...
    public void run() {
        ...
        // Integer key encodes character index of word concatenated with the
        ...
        for (int i = 0; i < list.getSize(); i++) {
            ...
            // if any of the words are found to be restricted, that means the
            // list is already sorted
            // stop doing further work
            if (w.state == XWord.RESTRICTED_STATE)
                break;

            // if any of the letters in the word are banned (have 0 orthogonal
            // combinations), it is marked as restricted
            ...
            // if the word has been marked as restricted, add it to the list and
            // go to the next word
            if (w.state == XWord.RESTRICTED_STATE) {
                scoredWords.add(w);
                continue;
            ...
            // find the word on the board, iterate across each letter, and then
            ...
            for (int j = 0; j < w.word.length(); j++) {
                ...
                // check the caches to see if this letter at this position has
                // already been scored
                if (verifiedChars.get(j).contains(c)) {
                    w.score *= scores.get(j * 100 + c);
                    if (focused_isHorizontal)
                        col++;
                    else
                        row++;
                    continue;
                ...
                // temporarily lock the letter and then run a query on the query
                // string perpendicular to this cell
