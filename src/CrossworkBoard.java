...
public class CrossworkBoard extends Component {
            ...
            for (int j = 0; j < boardSize; j++) {
                ...
                if (numberDisplayMode) {
                    if ((boardState.get(j - 1, i) == boardState.END && boardState.get(j + 1, i) != boardState.END)
                            || (boardState.get(j, i - 1) == boardState.END
                                    && boardState.get(j, i + 1) != boardState.END)) {
    ...
    private void drawLetter(Graphics2D g2, char c, int col, int row, Color color) {
        ...
        g2.setColor(color);
        g2.drawString(c + "", col * GRID_SIZE + (GRID_SIZE - fm.charWidth(c)) / 2.0f,
    ...
    private void drawNumber(Graphics2D g2, int number, int col, int row, Color color) {
        ...
        Color origColor = g2.getColor();
        g2.setColor(color);
        g2.drawString(number + "", col * GRID_SIZE + 1, row * GRID_SIZE + fm.getAscent());
        g2.setColor(origColor);
        g2.setFont(font);
    }
    ...
    // deletes all letters in current word that are part of perpendicular words
    // who are not fully locked
    public void deleteLettersOfUnfinalizedWords() {
        ...
        while (boardState.get(col, row) != boardState.END) {
            ...
            // check if letters in the perpendicular word are finalized (all
            // filled-in, all locked)
    ...
    public void showValidLettersForCurrentCell() {
        if (autocompleteFocusMode)
            return;
        if (boardState.getState(sel_x, sel_y) == boardState.WORD_STATE_LOCKED)
            return;
        ...
        if (goodChars.contains('A'))
            goodVowelCount++;
        if (goodChars.contains('E'))
            goodVowelCount++;
        if (goodChars.contains('I'))
            goodVowelCount++;
        if (goodChars.contains('O'))
            goodVowelCount++;
        if (goodChars.contains('U'))
            goodVowelCount++;
        if (goodChars.contains('Y'))
            goodVowelCount++;
        ...
        if (goodVowelCount == 6) {
            ...
            if (goodChars.contains('A'))
                wordList.addElement(new XWord("A"));
            if (goodChars.contains('E'))
                wordList.addElement(new XWord("E"));
            if (goodChars.contains('I'))
                wordList.addElement(new XWord("I"));
            if (goodChars.contains('O'))
                wordList.addElement(new XWord("O"));
            if (goodChars.contains('U'))
                wordList.addElement(new XWord("U"));
            if (goodChars.contains('Y'))
                wordList.addElement(new XWord("Y"));
        }
    ...
    private void advanceCursor(boolean horizontal, boolean forward, boolean skipBlocks) {
        if (!skipBlocks && boardState.get(horizontal ? (forward ? sel_x + 1 : sel_x - 1) : sel_x,
                !horizontal ? (forward ? sel_y + 1 : sel_y - 1) : sel_y) == boardState.END)
            return;
        if (horizontal) {
            do {
                sel_x += forward ? 1 : -1;
    ...
    private void saveUndoState() {
        if (autocompleteFocusMode)
            return;
        if (undoStack.size() > 0
                && undoStack.get(undoStack.size() - 1).getStateString().equals(boardState.getStateString()))
            return;
    ...
    private void sortWordList(DefaultListModel<XWord> list) {
        isLoading = true;
        WordListSorterThread t = new WordListSorterThread(list, boardState, finder, focused_isHorizontal, focused_x,
                focused_y, progressListener);
        t.start();
    }
    ...
    private void loadSaveStates() {
        ...
        try {
            BufferedReader r = new BufferedReader(
                    new InputStreamReader(new DataInputStream(new FileInputStream(saveFile))));
    ...
    public void setListAccessory(JList<XWord> la) {
        ...
        la.addKeyListener(new KeyListener() {
            ...
            public void keyPressed(KeyEvent e) {
                ...
                // INSERT
                if ((keyCode == 155) || (keyCode == 32 && (e.getModifiers() & 3) == 1)) {
        ...
        la.addListSelectionListener(new ListSelectionListener() {
            ...
            public void valueChanged(ListSelectionEvent e) {
                if (!autocompleteFocusMode)
                    return;
                if (listAccessory.getSelectedIndex() < 0)
                    return;
                if (listAccessory.getModel().getElementAt(0).equals(NO_MATCHES_MSG))
                    return;
        ...
        field.addKeyListener(new KeyListener() {
            ...
            public void keyPressed(KeyEvent e) {
                ...
                if (keyCode == 10) {
                    ...
                    listAccessory.setModel(wordList);
                    listAccessory
                            .setPreferredSize(new Dimension(listAccessory.getWidth() - 24, results.size() * GRID_SIZE));
                    listAccessory.revalidate();
                }
